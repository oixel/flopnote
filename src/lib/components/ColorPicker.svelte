<script lang="ts">
    import type { BrushHandler } from "$lib/scripts/BrushHandler.svelte";
    import {
        Color,
        colorToHex,
        hexToColor,
    ...
    let prevColorInput: string = $derived(colorToHex(color));
</script>
...
>
    <input
        ...
        oninput={(event) => {
            color = hexToColor(event.currentTarget.value, color.a);
        }}
